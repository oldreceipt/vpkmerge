...
use vpkmerge_core::{
    bake_uv_atlas, bake_uv_mask, edit_model_geometry, embed_metadata, export_femodel_json,
...
enum Command {
    ...
    /// Patch an identity file (`addoninfo.txt`) into an already-built VPK that
    /// has no way to trace it back to its source, e.g. a `GameBanana` download
    /// or one of this project's own older releases with no embedded metadata.
    /// Mirrors the classic Source engine "`AddonInfo`" `KeyValues` block,
    /// extended with a few `GameBanana` fields. Every original entry is
    /// preserved unchanged; only `addoninfo.txt` is added or replaced.
...
#[derive(Args)]
struct MetadataCmd {
    /// VPK to patch. Every existing entry is preserved unchanged; only
    /// `addoninfo.txt` is added or replaced.
    #[arg(long, value_name = "VPK")]
    vpk: PathBuf,

    /// Addon title (`addontitle`). Optional: omit it (with `--author`) when
    /// bringing your own `addoninfo.txt` via `--extra-file`.
    #[arg(long, value_name = "TITLE")]
    title: Option<String>,

    /// Addon author (`addonauthor`). Optional, like `--title`.
    #[arg(long, value_name = "AUTHOR")]
    author: Option<String>,

    /// Addon version (`addonversion`). Defaults to "1.0" if not given.
    #[arg(long, value_name = "VERSION")]
    version: Option<String>,

    /// Short description (`addonDescription`).
    #[arg(long, value_name = "TEXT")]
    description: Option<String>,

    /// Numeric `GameBanana` submission id this VPK was downloaded from, kept
    /// as a string so it round-trips verbatim.
    #[arg(long = "gamebanana-id", value_name = "ID")]
    gamebanana_id: Option<String>,

    /// Source page URL, e.g. the `GameBanana` listing this VPK came from.
    #[arg(long = "source-url", value_name = "URL")]
    source_url: Option<String>,

    /// Build timestamp, e.g. an ISO 8601 string from the caller's own clock.
    /// Not generated by this tool.
    #[arg(long = "build-date", value_name = "DATE")]
    build_date: Option<String>,

    /// Embed an extra arbitrary file alongside `addoninfo.txt`: `--extra-file
    /// ENTRY=LOCALPATH` (repeatable). LOCALPATH is read from disk and written at
    /// the VPK entry path ENTRY (subdirectories allowed). On a collision with an
    /// existing entry the extra file wins. Lets a caller embed its own files
    /// (e.g. `grimoire_meta.json`) without this tool understanding them.
    #[arg(long = "extra-file", value_name = "ENTRY=PATH")]
    extra_file: Vec<String>,
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Some(Command::Metadata(args)) => run_metadata(&args),
        None => run_merge(cli),
    }
}
...
/// Parse repeatable `--extra-file ENTRY=LOCALPATH` flags, reading each
/// LOCALPATH from disk into bytes. Mirrors the `--set ENTRY=PNG` house style:
/// split on the first `=`, both sides required. Returns `(entry_path, bytes)`
/// pairs ready to hand to the engine, which embeds them opaquely.
fn read_extra_files(specs: &[String]) -> Result<Vec<(String, Vec<u8>)>> {
    ...
    for spec in specs {
        ...
        let bytes = std::fs::read(local).with_context(|| format!("reading extra file {local}"))?;
        out.push((entry.to_string(), bytes));
    }
    Ok(out)
}

/// Print a one-line confirmation of the embedded extra entries, if any.
fn report_extra_files(entries: &[String]) {
    if !entries.is_empty() {
        println!(
            "  embedded {} extra file{}: {}",
            entries.len(),
...
fn run_merge(cli: Cli) -> Result<()> {
    ...
    let extra_files = read_extra_files(&cli.extra_file)?;
...
/// Patch identity metadata (`addoninfo.txt`) into an already-built VPK.
fn run_metadata(args: &MetadataCmd) -> Result<()> {
    // The typed addoninfo is optional. A caller that brings its own addoninfo.txt
    // via --extra-file (e.g. Grimoire, which serializes its own) passes no typed
    // fields, and we embed only the extra files. CLI users patching a mystery VPK
    // pass --title/--author for a generated addoninfo.txt.
    let has_typed = args.title.is_some()
        || args.author.is_some()
        || args.version.is_some()
        || args.description.is_some()
        || args.gamebanana_id.is_some()
        || args.source_url.is_some()
        || args.build_date.is_some();
    let metadata = has_typed.then(|| AddonMetadata {
        title: args.title.clone().unwrap_or_default(),
        author: args.author.clone().unwrap_or_default(),
        version: args.version.clone(),
        description: args.description.clone(),
        gamebanana_id: args.gamebanana_id.clone(),
