...
//! target while preserving its saturation and value, then re-encodes the full
//! mip chain in the texture's own format via [`morphic::replace_mip_chain`].
...
/// Decode the top mip of a `.vtex_c` and set every pixel's hue to `hue_deg`
/// (preserving each pixel's saturation and value), returning the recolored
/// image *without* re-encoding.
///
...
pub fn recolor_texture_image(vtex_bytes: &[u8], hue_deg: f64) -> Result<Image> {
    let mut image = morphic::decode(vtex_bytes).context("decoding texture top mip")?;
    shift_hue_in_place(&mut image, hue_deg)?;
    Ok(image)
}
...
pub fn recolor_texture_hue(vtex_bytes: &[u8], hue_deg: f64) -> Result<Vec<u8>> {
    let recolored = recolor_texture_image(vtex_bytes, hue_deg)?;
...
pub fn recolor_texture_preview_png(vtex_bytes: &[u8], hue_deg: f64) -> Result<Vec<u8>> {
    let image = recolor_texture_image(vtex_bytes, hue_deg)?;
...
/// Recolor every baked per-vertex `COLOR` of a model's mesh buffers to
/// `hue_deg`, keeping each vertex's saturation and value, and return the new
/// `.vmdl_c` bytes.
///
...
pub fn recolor_model_vertex_colors(
    ...
    // The same display-space hue set as the texture/particle recolor: vertex
    // colors are 8-bit unorm, so round-tripping each channel through the u8
    // `set_hue` is lossless and keeps all three recolor paths on one transform.
...
fn shift_hue_in_place(image: &mut Image, hue_deg: f64) -> Result<()> {
    match &mut image.data {
        ImageData::Rgba8(buf) => {
            for px in buf.chunks_exact_mut(4) {
                let [r, g, b] = set_hue([px[0], px[1], px[2]], hue_deg);
                px[0] = r;
                px[1] = g;
                px[2] = b;
...
/// One pixel: set its hue to `hue_deg` (taken mod 360), keeping saturation and
/// value.
#[allow(clippy::many_single_char_names)]
fn set_hue(rgb: [u8; 3], hue_deg: f64) -> [u8; 3] {
    let (_, s, v) = rgb_to_hsv(
        f64::from(rgb[0]) / 255.0,
        f64::from(rgb[1]) / 255.0,
        f64::from(rgb[2]) / 255.0,
    );
    let (r, g, b) = hsv_to_rgb(hue_deg.rem_euclid(360.0), s, v);
...
mod tests {
    ...
    fn recolor_image_sets_hue_keeps_sat_and_value() {
        ...
        let target = 200.0;
        let recolored = recolor_texture_image(&bytes, target).expect("recolor");
    ...
    fn recolor_hue_round_trips_to_loadable_vtex() {
        ...
        let out = recolor_texture_hue(&bytes, 120.0).expect("recolor to vtex");
    ...
    fn hdr_texture_is_rejected() {
        ...
        let err = recolor_texture_hue(&bytes, 90.0).expect_err("HDR must be rejected");
