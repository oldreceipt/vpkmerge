// Animate a prism-recolored Yamato VPK: make its spectrum color gradients CYCLE by
// flipping each age-driven gradient driver to looped collection-age. Uses the
// byte-faithful string-table append (morphic::patch_kv3_resource_strings_adding via
// vpkmerge_core::loop_animate_particle_bytes), so the compiled particles stay
// engine-loadable. Re-packs EVERY particle (looped where applicable, otherwise the
// input bytes unchanged), so the output is a complete drop-in replacement for the
// prism VPK.
//
// usage:
//   cargo run -p vpkmerge-core --example yamato_loop_anim -- \
//     <yamato_prism_dir.vpk> <out_loop_dir.vpk>
fn main() -> anyhow::Result<()> {
    let mut args = std::env::args().skip(1);
    let input = args.next().expect("yamato_prism_dir.vpk");
    let out = args.next().expect("out_loop_dir.vpk");

    let vpk = valve_pak::open(&input)?;
    let mut entries: Vec<String> = vpk
        .file_paths()
        .filter(|p| p.ends_with(".vpcf_c"))
        .cloned()
        .collect();
    entries.sort();

    let mut packed: Vec<(String, Vec<u8>)> = Vec::new();
    let mut looped = 0usize;
    let mut skipped_err = 0usize;
    for entry in &entries {
        let bytes = vpk.get_file(entry)?.read_all()?;
        match vpkmerge_core::loop_animate_particle_bytes(&bytes) {
            Ok(Some(new_bytes)) => {
                packed.push((entry.clone(), new_bytes));
                looped += 1;
            }
            // No loopable gradient: keep the (prism-colored) bytes as-is.
            Ok(None) => packed.push((entry.clone(), bytes)),
            // A particle the string-add patch could not touch (e.g. a v4 block missing
            // the enum string): leave it prism-colored but unlooped rather than abort.
            Err(e) => {
                skipped_err += 1;
                eprintln!("  note: not looping {entry} (left as prism): {e:#}");
                packed.push((entry.clone(), bytes));
            }
        }
    }

    let refs: Vec<(&str, &[u8])> = packed
        .iter()
        .map(|(p, b)| (p.as_str(), b.as_slice()))
        .collect();
    vpkmerge_core::pack(&refs, &out)?;
    println!(
        "wrote {out}: {} particles, {looped} looped (spectrum now cycles), {skipped_err} left unlooped",
        refs.len()
    );
    Ok(())
}
